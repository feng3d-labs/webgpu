...
{
    const updateBufferBindingKey: UpdateBufferBindingKey = [uniformData, type];
    let result = updateBufferBindingMap.get(updateBufferBindingKey);
    if (result) return result.value;
    ...
    updateBufferBindingMap.set(updateBufferBindingKey, result);
