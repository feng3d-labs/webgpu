...
{
    device.buffers ??= new WeakMap();

    let result = device.buffers.get(buffer);
    ...
    result = computed(() =>
    {
        ...
        gpuBuffer = device.createBuffer({
            label,
            size,
            usage: usage ?? defaultGPUBufferUsage,
            mappedAtCreation,
        });
        ...
        onWriteBuffers(device, gpuBuffer, buffer);

        // 更新GPU缓冲区引用
        return gpuBuffer;
    });

    device.buffers?.set(buffer, result);

    return result.value;
}

function onWriteBuffers(device: GPUDevice, gpuBuffer: GPUBuffer, buffer: Buffer)
{
    const r_buffer = reactive(buffer);
...
declare global
{
    ...
    {
        /** 缓冲区实例缓存映射表 */
        buffers: WeakMap<Buffer, Computed<GPUBuffer>>;
    }
}
