...
本包发布在 **GitHub Packages**（而非公共 npm 注册表）。安装前需要先配置注册表认证。
...
```
@feng3d-labs:registry=https://npm.pkg.github.com
//npm.pkg.github.com/:_authToken=${GITHUB_TOKEN}
```

并设置环境变量 `GITHUB_TOKEN` 为一个具有 `read:packages` 权限的 GitHub Personal Access Token。

### 2. 安装

发布的真实包名是 `@feng3d-labs/webgpu`，建议使用 npm 别名映射回 `@feng3d/webgpu` 以保持 import 语句简短：

```json
{
  "dependencies": {
    "@feng3d/webgpu": "npm:@feng3d-labs/webgpu@*"
  }
}
```
...
```bash
npm install @feng3d-labs/webgpu
```
